...
type Parameters = {
	amount: number;
	funding_source: FundingSourceTypes;
	pin: string;
	currency: string;
};
...
export async function production({ currency, ...data }: Parameters): Promise<Response> {
	const response = await axios.post(`/investment/fund-vault?${currency}`, data);
...
export default async function fundVault(data: Parameters): Promise<Response> {
