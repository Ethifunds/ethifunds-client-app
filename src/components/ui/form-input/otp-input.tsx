...
export default function PinInput(props: PinInputProps) {
	const { onChange, value, valueLength, inputClass: classes } = props;

	const inputClass = `${classes} border-black rounded-md font-bold mx-1 w-full text-center bg-white border text-xl`;
	...
	const inputOnKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, idx: number) => {
		...
		if ((key === "Backspace" && nextInputEl?.value === "") || idx === valueLength - 1) {
	...
	const output = valueItems.map((digit, idx) => {
		return (
			<input
				...
				className={`${inputClass} ${digit ? "!bg-white !border-2 !border-primary": "!bg-neutral-200"} `}
				type="text"
				key={idx}
				inputMode="numeric"
				pattern="\d{1}"
	...
	return <div className="w-full flex justify-center items-center">{output}</div>;
}
