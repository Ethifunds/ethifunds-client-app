...
export default function useForm() {
  ...
  const { data: savedEmail } = useStorage(
  ...
  const query = useQuery(
    ...
    () => getUserSecurityQuestions({ email: userEmail }),
    {
  ...
  const updateForm = (e: React.ChangeEvent<HTMLInputElement>, idx: number) => {
    ...
    setFormData((prev) =>
      prev.map((item, index) =>
        index === idx
          ? { ...item, question_id: securityQuestions[idx].id, answer: value }
          : item,
      ),
    );
  };
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (
        formData.some((item) => item.answer.length < 1 || item.question_id < 1)
      ) {
