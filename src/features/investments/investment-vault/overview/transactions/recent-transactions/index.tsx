		...
		<div className="space-y-5 ">
			<ErrorBoundary>
				<div className="flex justify-between items-center px-1">
					...
					{props.data.length > 0 && (
						<Link to={"/investments/vault/transactions"} className="underline text-primary">
							View All
						</Link>
					)}
				</div>

				<TransactionFilters disabled={!props.data.length} />

				<div className="h-full min-h-60 max-h-96 overflow-auto">
					<TransactionTable data={props.data} isEmpty={!props.data?.length} sign={currency.code} />
				</div>
			</ErrorBoundary>
		</div>
	);
